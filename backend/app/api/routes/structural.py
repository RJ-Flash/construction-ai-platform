"""
Structural API Routes

This module contains the API routes for structural plugins.
"""
...
async def list_structural_plugins(current_user: User = Depends(get_current_user)):
    """
    List all available structural plugins.
    """
    # Get all structural plugins
    plugins = get_plugins_by_category("structural")
...
async def analyze_with_plugin(
    ...
    """
    Analyze text using a specific structural plugin.
    """
    # Get the plugin
    ...
    # Check if it's a structural plugin
    plugin = plugin_class()
    if plugin.category != "structural":
        raise HTTPException(status_code=400, detail=f"Plugin '{plugin_id}' is not a structural plugin")
