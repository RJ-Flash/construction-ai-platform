...
This module contains the API routes for cost estimation plugins.
"""
...
async def list_cost_plugins(current_user: User = Depends(get_current_user)):
    """
    List all available cost estimation plugins.
    """
    # Get all cost plugins
    plugins = get_plugins_by_category("cost")
...
async def analyze_with_plugin(
    ...
    """
    Analyze text using a specific cost estimation plugin.
    """
    # Get the plugin
    ...
    # Check if it's a cost plugin
    plugin = plugin_class()
    if plugin.category != "cost":
        raise HTTPException(status_code=400, detail=f"Plugin '{plugin_id}' is not a cost estimation plugin")
